...
import ttit.com.shuvo.terrainerp.adapters.CashTransactionAdapter;
import ttit.com.shuvo.terrainerp.arrayList.CashTransactionLists;
...
//    private Connection connection;
    ...
    ArrayList<StringWthBool> salesOrder;
...
//    }
    ...
    public void onCategoryClicked(int CategoryPosition) {
        ...
        if (cashTransactionLists.get(CategoryPosition).getType().equals("AV")) {
            ...
            fromCS = 1;
            DebitCreditVoucher debitCreditVoucher = new DebitCreditVoucher(mContext);
            ...
            VM_NO = cashTransactionLists.get(CategoryPosition).getInvPurNo();
            fromCS = 1;
            DirectPurchase directPurchase = new DirectPurchase(mContext);
            ...
            for (int i = 0; i < deliveryChallanListsCS.size(); i++) {
                if (orderNo.equals(deliveryChallanListsCS.get(i).getDelivery_no())) {
                    ...
                    fromCS = 1;
                    DeliveryChallanDetails deliveryChallanDetails = new DeliveryChallanDetails(mContext);
...
//    }
    ...
    public void getTransData() {
        ...
        String cashTransUrl = "http://103.56.208.123:8001/apex/tterp/cashTranx/getCashTranxList/"+firstDate+"/"+lastDate+"";

        RequestQueue requestQueue = Volley.newRequestQueue(mContext);

        StringRequest cahsTransReq = new StringRequest(Request.Method.GET, cashTransUrl, response -> {
            conn = true;
            try {
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        String cash_tranx = info.getString("cash_tranx");
                        JSONArray ledgerArray = new JSONArray(cash_tranx);
                        for (int j = 0; j < ledgerArray.length(); j++) {
                            JSONObject ledger_info = ledgerArray.getJSONObject(j);

                            String cid_name = ledger_info.getString("cid_name");
                            ...
                            String total_dr = ledger_info.getString("total_dr")
                                    .equals("null") ? "0" : ledger_info.getString("total_dr");
                            String total_cr = ledger_info.getString("total_cr")
                                    .equals("null") ? "0" : ledger_info.getString("total_cr");
                            String balance = ledger_info.getString("balance")
                                    .equals("null") ? "0" : ledger_info.getString("balance");
                            String previous_balance = ledger_info.getString("previous_balance")
                                    .equals("null") ? "0" : ledger_info.getString("previous_balance");
                            String total_balance = ledger_info.getString("total_balance")
                                    .equals("null") ? "0" : ledger_info.getString("total_balance");


                            if (lg_trans_type.equals("SOM")) {
                                salesOrder.add(new StringWthBool(lg_inv_pur_no,false));
                            }
                            if (lg_trans_type.equals("SM")) {
                                deliveryChallanNo.add(new StringWthBool(lg_inv_pur_no,false));
                            }

                            cashTransactionLists.add(new CashTransactionLists(cid_name, lg_voucher_no,lg_particulars,
                                    lg_dr_amt,lg_cr_amt,lg_inv_pur_no,lg_trans_type,total_dr,total_cr,
                                    balance,previous_balance,total_balance));
                        }
                    }
                }
                getSOMLists();
        ...
        requestQueue.add(cahsTransReq);

    }

    public void getSOMLists() {
        if (salesOrder.size() != 0) {
            boolean allUpdated = false;
            for (int i = 0 ; i < salesOrder.size(); i++) {
                allUpdated = salesOrder.get(i).isUpdated();
                if (!salesOrder.get(i).isUpdated()) {
                    allUpdated = salesOrder.get(i).isUpdated();
                    String som_id = salesOrder.get(i).getName();
                    getSalesOrderLists(som_id, i);
                    break;
                }
            }
            if (allUpdated) {
    ...
    public void getSalesOrderLists(String som_id, int index) {
        String url = "http://103.56.208.123:8001/apex/tterp/cashTranx/getSomOrderLists?som_id="+som_id+"";
            ...
            try {
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        String som_id_new = info.getString("som_id");
                        ...
                        salesOrderListsCS.add(new SalesOrderList(som_id_new, ad_id,ad_name,
                ...
                salesOrder.get(index).setUpdated(true);
                getSOMLists();
            }
            catch (JSONException e) {
                e.printStackTrace();
        ...
        if (deliveryChallanNo.size() != 0) {
            ...
            for (int i = 0 ; i < deliveryChallanNo.size(); i++) {
                ...
                if (!deliveryChallanNo.get(i).isUpdated()) {
                    allUpdated = deliveryChallanNo.get(i).isUpdated();
                    String sm_id = deliveryChallanNo.get(i).getName();
                    getDeliveryOrderLists(sm_id, i);
                    break;
                }
            }
            if (allUpdated) {
    ...
    public void getDeliveryOrderLists(String sm_id, int index) {
        String url = "http://103.56.208.123:8001/apex/tterp/cashTranx/getSM_OrderLists?sm_id="+sm_id+"";
            ...
            try {
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        String sm_id_new = info.getString("sm_id");
                        ...
                        deliveryChallanListsCS.add(new DeliveryChallanList(sm_id_new, sm_delivery_no,sm_date,
                                ad_name, order_type,om_order_no, om_odr_date,sm_invoice_amt, sm_vat_amt,
                                ad_id,edd,ad_code, target_address,aad_contact_person,aad_contact_no, aad_email));
                    }
                }

                deliveryChallanNo.get(index).setUpdated(true);
                getSMLists();
            }
            catch (JSONException e) {
                e.printStackTrace();
