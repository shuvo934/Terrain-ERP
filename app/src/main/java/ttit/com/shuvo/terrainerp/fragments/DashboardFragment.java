...
//        lineChart.setMarker(mv);
        ...
        selectYear.setText("Year: "+ String.valueOf(formattedYear));
...
//    }
    ...
    public void getData() {
        ...
        String workOrderUrl = "http://103.56.208.123:8001/apex/tterp/dashboard/getCategoryWiseWorkOrderAmnt/"+firstDate+"/"+lastDate+"";
        String rcvUrl = "http://103.56.208.123:8001/apex/tterp/dashboard/getCategoryWiseRcvAmnt?st_date="+firstDate+"&end_date="+lastDate+"&ad_id=&rm_type_flag=";
        String saleOrdUrl = "http://103.56.208.123:8001/apex/tterp/dashboard/getCategoryWiseOrderAmnt?st_date="+firstDate+"&end_date="+lastDate+"&ad_id=";
        String devOrderUrl = "http://103.56.208.123:8001/apex/tterp/dashboard/getCategoryWiseSalesAmnt?st_date="+firstDate+"&end_date="+lastDate+"&ad_id=";
        String custSuppUrl = "http://103.56.208.123:8001/apex/tterp/dashboard/getCustomerAndSupplier";
        String monthWiseSaleUrl = "http://103.56.208.123:8001/apex/tterp/dashboard/getMonthWiseSalesWithOption_1?st_date="+firstDate+"&end_date="+lastDate+"&im_id=&subcatm_id=";

        RequestQueue requestQueue = Volley.newRequestQueue(mContext);

        StringRequest monthlySaleReq = new StringRequest(Request.Method.GET, monthWiseSaleUrl, response -> {
            conn = true;
            try {
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        String month_id = info.getString("month_id");
                        String amt = info.getString("amt");

                        cateValuesDD.add(new ChartValue(month_id,month_name,amt));
                    }
                }
                connected = true;
                updateFragment();
        ...
        StringRequest custSuppReq = new StringRequest(Request.Method.GET, custSuppUrl, response -> {
            conn = true;
            try {
                JSONObject jsonObject = new JSONObject(response);
                String items = jsonObject.getString("items");
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        JSONObject info = array.getJSONObject(i);
                        supplier_total = info.getInt("total_supplier");
                        customer_total = info.getInt("total_customer");
                    }
                }

                requestQueue.add(monthlySaleReq);
            }
            catch (JSONException e) {
                e.printStackTrace();
                connected = false;
        ...
        StringRequest devOrdReq = new StringRequest(Request.Method.GET, devOrderUrl, response -> {
            conn = true;
            try {
                JSONObject jsonObject = new JSONObject(response);
                String items = jsonObject.getString("items");
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        JSONObject info = array.getJSONObject(i);
                        int total_sales_without_vat = info.getInt("total_sales_without_vat");
                        delivery_total = delivery_total + total_sales_without_vat;
                    }
                }
                requestQueue.add(custSuppReq);
            }
            catch (JSONException e) {
                e.printStackTrace();
                connected = false;
        ...
        StringRequest salesOrdReq = new StringRequest(Request.Method.GET, saleOrdUrl, response -> {
            conn = true;
            try {
                JSONObject jsonObject = new JSONObject(response);
                String items = jsonObject.getString("items");
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        JSONObject info = array.getJSONObject(i);
                        int total_order = info.getInt("total_order");
                        sales_order_total = sales_order_total + total_order;
                    }
                }

                requestQueue.add(devOrdReq);
            }
            catch (JSONException e) {
                e.printStackTrace();
                connected = false;
        ...
        StringRequest rcvReq = new StringRequest(Request.Method.GET, rcvUrl, response -> {
            ...
            try {
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        JSONObject info = array.getJSONObject(i);
                        int total_amt = info.getInt("total_amt");
                        receive_total = receive_total + total_amt;
                    }
                }

                requestQueue.add(salesOrdReq);
            }
            catch (JSONException e) {
                e.printStackTrace();
                connected = false;
        ...
        StringRequest woReq = new StringRequest(Request.Method.GET, workOrderUrl, response -> {
            conn = true;
            try {
                JSONObject jsonObject = new JSONObject(response);
                String items = jsonObject.getString("items");
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        JSONObject info = array.getJSONObject(i);
                        int total_job_amt = info.getInt("total_job_amt");
                        work_order_total = work_order_total + total_job_amt;
                    }
                }
