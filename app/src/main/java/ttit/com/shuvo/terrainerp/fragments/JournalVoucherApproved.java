...
import ttit.com.shuvo.terrainerp.adapters.JournalVoucherApprovedAdapter;
 ...
 * Use the {@link JournalVoucherApproved#newInstance} factory method to
 * create an instance of this fragment.
 */
public class JournalVoucherApproved extends Fragment implements JournalVoucherApprovedAdapter.ClickedItem{
...
//    private Connection connection;
    ...
    public static TextInputEditText journalVoucherSelectSpinner;

    public static String jVm_id = "";

    public static int fromJVA = 0;

    public static String status_approved_JV = "";

    public static String appDisUser_JV = "";
    public static String appDisTime_JV = "";
    ...
    ArrayList<CreditVoucherApprovedLists> journalVoucherApprovedLists;

    double total_debit = 0.0;
    double total_credit = 0.0;
    String approve_flag = "";

    public JournalVoucherApproved() {
     ...
     * @return A new instance of fragment JournalVoucherApproved.
     */
    // TODO: Rename and change types and number of parameters
    public static JournalVoucherApproved newInstance(String param1, String param2) {
        JournalVoucherApproved fragment = new JournalVoucherApproved();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_journal_voucher_approved, container, false);

        journalVoucherSelectSpinner = view.findViewById(R.id.journal_voucher_search_text);
        afterVoucherSelect = view.findViewById(R.id.journal_voucher_approved_details_card);
        afterVoucherSelect.setVisibility(View.GONE);

        vDate = view.findViewById(R.id.date_journal_approved);
        vTime = view.findViewById(R.id.time_journal_approved);
        preparedBy = view.findViewById(R.id.prepared_by_journal_approved);
        billRefNo = view.findViewById(R.id.bill_ref_no_journal_approved);
        billRefDate = view.findViewById(R.id.bill_ref_date_journal_approved);
        narration = view.findViewById(R.id.narration_journal_approved);
        statusReview = view.findViewById(R.id.status_journal_approved);
        appOrDisAppUser = view.findViewById(R.id.app_or_disapp_journal_approved);
        appOrDisAppUserLay = view.findViewById(R.id.app_or_disapp_journal_approved_layout);
        appOrDisAppTime = view.findViewById(R.id.app_or_disapp_time_journal_approved);
        appOrDisAppTimeLay = view.findViewById(R.id.app_or_disapp_time_journal_approved_layout);

        totalDebit = view.findViewById(R.id.total_debit_jva);
        totalCredit = view.findViewById(R.id.total_credit_jva);

        approve = view.findViewById(R.id.approve_button_jva);
        reject = view.findViewById(R.id.reject_button_jva);

        itemView = view.findViewById(R.id.journal_voucher_approved_transaction_view);
        ...
        journalVoucherApprovedLists = new ArrayList<>();

        journalVoucherSelectSpinner.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                fromJVA = 1;
                VoucherSelectionDial voucherSelectionDial = new VoucherSelectionDial(mContext);
                voucherSelectionDial.show(getActivity().getSupportFragmentManager(),"JVS");
            }
        });

        journalVoucherSelectSpinner.addTextChangedListener(new TextWatcher() {
            @Override
...
//                    System.out.println(status_approved_JV);
...
//
//                journalVoucherApprovedAdapter = new JournalVoucherApprovedAdapter(journalVoucherApprovedLists,getContext(),JournalVoucherApproved.this);
//                itemView.setAdapter(journalVoucherApprovedAdapter);
//
//                if (journalVoucherApprovedLists.size() != 0) {
//                    vDate.setText(journalVoucherApprovedLists.get(0).getVmDate());
//                    vTime.setText(journalVoucherApprovedLists.get(0).getVmTime());
//                    preparedBy.setText(journalVoucherApprovedLists.get(0).getVmUser());
//                    billRefNo.setText(journalVoucherApprovedLists.get(0).getVmBillRefNo());
//                    billRefDate.setText(journalVoucherApprovedLists.get(0).getVmBillRefDate());
//                    narration.setText(journalVoucherApprovedLists.get(0).getVmNarration());
//                } else {
//                    vDate.setText("");
//                    vTime.setText("");
//                    preparedBy.setText("");
...
//                afterVoucherSelect.setVisibility(View.GONE);
//                jVm_id = "";
//                journalVoucherSelectSpinner.setText("");
//
//
//
//
...
//            callableStatement1.setInt(2,Integer.parseInt(jVm_id));
...
//                journalVoucherApprovedLists.add(new CreditVoucherApprovedLists(resultSet.getString(1),resultSet.getString(2),resultSet.getString(3),
...
//
//            if (journalVoucherApprovedLists.size() != 0) {
//                for (int i = 0; i < journalVoucherApprovedLists.size() ; i++) {
//                    if (journalVoucherApprovedLists.get(i).getVdDebit() != null) {
//                        total_debit = total_debit + Double.parseDouble(journalVoucherApprovedLists.get(i).getVdDebit());
//                    }
//                    if (journalVoucherApprovedLists.get(i).getVdCredit() != null) {
//                        total_credit = total_credit + Double.parseDouble(journalVoucherApprovedLists.get(i).getVdCredit());
//                    }
//                }
//            }
//
...
//    }
    ...
    public void getAccData() {
        ...
        String url = "http://103.56.208.123:8001/apex/tterp/voucherApprove/getVoucherList/"+jVm_id+"";
        ...
        StringRequest request = new StringRequest(Request.Method.GET, url, response -> {
            ...
            try {
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        for (int j = 0; j < voucherArray.length(); j++) {
                            ...
                            journalVoucherApprovedLists.add(new CreditVoucherApprovedLists(vm_id,vm_no,vm_date,
                ...
                if (journalVoucherApprovedLists.size() != 0) {
                    for (int i = 0; i < journalVoucherApprovedLists.size() ; i++) {
                        if (journalVoucherApprovedLists.get(i).getVdDebit() != null) {
                            if (!journalVoucherApprovedLists.get(i).getVdDebit().isEmpty()) {
                                total_debit = total_debit + Double.parseDouble(journalVoucherApprovedLists.get(i).getVdDebit());
                            }
                        }
                        if (journalVoucherApprovedLists.get(i).getVdCredit() != null) {
                            if (!journalVoucherApprovedLists.get(i).getVdCredit().isEmpty()) {
                                total_credit = total_credit + Double.parseDouble(journalVoucherApprovedLists.get(i).getVdCredit());
                            }
                        }
                    }
...
//            callableStatement1.setInt(1,Integer.parseInt(jVm_id));
    ...
    private void updateLayout() {
        ...
        if (conn) {
            if (connected) {
                ...
                AlertDialog dialog = new AlertDialog.Builder(mContext)
                        .setMessage("Failed to update Journal Voucher. Please Try Again.")
                        .setPositiveButton("Retry", null)
                        .setNegativeButton("Cancel", null)
                        .show();
