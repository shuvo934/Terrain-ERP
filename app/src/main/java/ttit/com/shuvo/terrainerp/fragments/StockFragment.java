...
import ttit.com.shuvo.terrainerp.adapters.ItemDetailsAdapter;
import ttit.com.shuvo.terrainerp.arrayList.ChartValue;
import ttit.com.shuvo.terrainerp.arrayList.ItemDetaisList;
...
//    }
    ...
    public void getWarehouses() {
        ...
        String url = "http://103.56.208.123:8001/apex/tterp/utility/getWarehouse";
            ...
            try {
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        String whd_rack_name = info.getString("whd_rack_name");
                        String sl_whm_id = info.getString("sl_whm_id");

                        wareHouseLists.add(new ReceiveTypeList(sl_whm_id,whd_rack_name,""));
                    }
                    wareHouseLists.add(new ReceiveTypeList("","All Warehouse",""));
                }
    ...
    public void getCatData(int parser) {
        categoryValues = new ArrayList<>();

        String url = "http://103.56.208.123:8001/apex/tterp/stockOverview/getWhWiseCatStk?whm_id="+warehouseId+"";
        RequestQueue requestQueue = Volley.newRequestQueue(mContext);
        ...
        StringRequest request = new StringRequest(Request.Method.GET, url, response -> {
            ...
            try {
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        JSONObject info = array.getJSONObject(i);
                        String wh_wise_category_stock = info.getString("wh_wise_category_stock");
                        JSONArray stockArray = new JSONArray(wh_wise_category_stock);
                        for (int j = 0; j < stockArray.length(); j++) {
                            JSONObject stock_info = stockArray.getJSONObject(j);

//                            String total_stock = stock_info.getString("total_stock");
                            String total_stock_val = stock_info.getString("total_stock_val");
                            String im_id = stock_info.getString("im_id");
                            String im_name = stock_info.getString("im_name");

                            categoryValues.add(new ChartValue(im_id, im_name,total_stock_val));
    ...
    private void updateCatValue() {
        ...
        String text = totalValue+" BDT";
        totalStock.setText(text);
    }
...
//    }
    ...
    public void getSubCategory() {
        ...
        String url = "http://103.56.208.123:8001/apex/tterp/stockOverview/getWhWiseSubCatStk?whm_id="+warehouseId+"&cat_id="+categoryId+"";
        RequestQueue requestQueue = Volley.newRequestQueue(mContext);
        ...
        StringRequest request = new StringRequest(Request.Method.GET, url, response -> {
            ...
            try {
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        JSONObject info = array.getJSONObject(i);
                        String wh_wise_subcategory_stock = info.getString("wh_wise_subcategory_stock");
                        JSONArray stockArray = new JSONArray(wh_wise_subcategory_stock);
                        for (int j = 0; j < stockArray.length(); j++) {
                            JSONObject stock_info = stockArray.getJSONObject(j);

//                            String total_stock = stock_info.getString("total_stock");
                            String total_stock_val = stock_info.getString("total_stock_val");
                            String isc_id = stock_info.getString("isc_id");
                            String subcatm_name = stock_info.getString("subcatm_name");

                            subCategoryValues.add(new ChartValue(isc_id, subcatm_name,total_stock_val));
    ...
    private void updateSubCatValue() {
        waitProgress.dismiss();
        if (conn) {
            if (connected) {
                conn = false;
...
//    }
    ...
    public void getItemData() {
        ...
        String url = "http://103.56.208.123:8001/apex/tterp/stockOverview/getWhWiseItmStk?whm_id="+warehouseId+"&sub_cat_id="+subCatrgoryId+"";
        RequestQueue requestQueue = Volley.newRequestQueue(mContext);
            ...
            try {
                ...
                if (!count.equals("0")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        JSONArray stockArray = new JSONArray(wh_wise_item_stock);
                        for (int j = 0; j < stockArray.length(); j++) {
                            JSONObject stock_info = stockArray.getJSONObject(j);
...
//                            String subcatm_name = stock_info.getString("subcatm_name");
                            ...
                            stockItemLists.add(new ItemDetaisList(String.valueOf(j+1),item_code,item_reference_name,item_unit,
                                    total_stock,vat_pct,total_stock_val,selling_price,item_hs_code));
